...
**前置条件：必须先确认 PDF 已下载到 `papers/<slug>/paper.pdf`，才能开始阅读和撰写 notes。**

- 如果 Step 1 未成功下载 PDF（文件不存在或为空），必须先重新获取 PDF，不可仅凭元数据或摘要写 notes。
- 如果 PDF 下载失败，在 metadata.json 的 `note` 字段标记 `"PDF unavailable"`，并跳过该论文的后续步骤。

确认 PDF 就位后，用 Read 工具读取 PDF（大文件用 pages 参数分页读取）。
